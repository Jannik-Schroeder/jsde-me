...
export default function Resume() {
  const resume = useMemo(() => {
    ...
    return roles.sort((a, b) => {
      ...
      const aEnd = typeof a.end === 'string' ? a.end : a.end.label
      const bEnd = typeof b.end === 'string' ? b.end : b.end.label

      // Wenn beide "Present" sind, sortiere nach Startdatum (neuestes zuerst)
      if (aEnd === 'Present' && bEnd === 'Present') {
        return new Date(bStart).getTime() - new Date(aStart).getTime()
      }

      // Wenn nur einer "Present" ist, dieser kommt zuerst
      if (aEnd === 'Present') return -1
      if (bEnd === 'Present') return 1

      // Ansonsten nach Startdatum sortieren
      return new Date(bStart).getTime() - new Date(aStart).getTime()
    })
  }, [])
